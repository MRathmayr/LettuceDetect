...
class GroundingProbeDetector(Stage3Detector):
    ...
    def __init__(
        ...
        # Load sklearn probe (local path or HuggingFace Hub)
        if probe_path:
            logger.info(f"Loading grounding probe: {probe_path}")
            self._probe = GroundingProbe.load(probe_path)
        elif probe_repo_id and probe_filename:
            logger.info(f"Loading grounding probe from HF: {probe_repo_id}/{probe_filename}")
            self._probe = GroundingProbe.from_hub(probe_repo_id, probe_filename)
        else:
            raise ValueError(
                "Either probe_path or (probe_repo_id + probe_filename) required. "
